...
function seedData() {
  ...
  const adminChecknumber = process.env.ADMIN_CHECKNUMBER;
  const adminPassword = process.env.ADMIN_PASSWORD;
  const adminName = process.env.ADMIN_NAME || 'System Administrator';
  if (!adminChecknumber || !adminPassword) {
    throw new Error('ADMIN_CHECKNUMBER and ADMIN_PASSWORD must be set before the first start.');
  }
  ...
  insertUser.run(adminChecknumber, adminPassword, adminName, 'admin', null);
